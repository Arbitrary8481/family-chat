...
function scrollToBottomRobust() {
    ...
    const container = document.getElementById('messagesContainer');
    if (!container) return;
    const rescrollIfStillNearBottom = () => {
        if (isNearBottom(container)) scrollToBottom();
    };
    container.querySelectorAll('img').forEach(img => {
        if (!img.complete) {
            img.addEventListener('load', rescrollIfStillNearBottom, { once: true });
            img.addEventListener('error', rescrollIfStillNearBottom, { once: true });
        }
    });
}
