...
def init_db():
    ...
    # message comes in — see notify_message_subscribers().
...
def get_notification_pref(user_id):
    conn = sqlite3.connect('/data/chat.db')
    c = conn.cursor()
    c.execute('SELECT enabled, notify_service FROM notification_prefs WHERE user_id = ?', (user_id,))
    row = c.fetchone()
    conn.close()
    if not row:
        return {'enabled': False, 'notify_service': ''}
    return {'enabled': bool(row[0]), 'notify_service': row[1] or ''}

def set_notification_pref(user_id, enabled, notify_service):
    notify_service = (notify_service or '').strip()[:100]
    conn = sqlite3.connect('/data/chat.db')
    c = conn.cursor()
    c.execute('''INSERT INTO notification_prefs (user_id, enabled, notify_service) VALUES (?, ?, ?)
                 ON CONFLICT(user_id) DO UPDATE SET
                    enabled = excluded.enabled, notify_service = excluded.notify_service''',
              (user_id, 1 if enabled else 0, notify_service))
    conn.commit()
    conn.close()

def get_subscribed_users(exclude_user_id=None):
    """(user_id, notify_service) for everyone who has notifications
    enabled AND has actually picked a device — excluding the given user,
    since a sender shouldn't be notified about their own message."""
    conn = sqlite3.connect('/data/chat.db')
    c = conn.cursor()
    c.execute('SELECT user_id, notify_service FROM notification_prefs WHERE enabled = 1')
    rows = c.fetchall()
    conn.close()
...
def notify_message_subscribers(channel, sender, sender_id, content, msg_type, file_info):
    ...
    if not SUPERVISOR_TOKEN:
        return
    subscribers = get_subscribed_users(exclude_user_id=sender_id)
    if not subscribers:
        return
...
def api_get_notify_prefs():
    ...
    if not ha_user_id:
        return jsonify({'error': 'Not accessed through Home Assistant'}), 403
    return jsonify(get_notification_pref(ha_user_id))
...
def api_set_notify_prefs():
    ...
    if enabled and not (notify_service or '').strip():
        return jsonify({'error': 'Choose a device before turning notifications on.'}), 400
    set_notification_pref(ha_user_id, enabled, notify_service)
    return jsonify({'success': True})
