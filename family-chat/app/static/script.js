...
'⛹️':'person bouncing ball','🏋️':'person lifting weights','🚴':'person biking','🚵':'person mountain biking','🎽':'running shirt','🎿':'skis','🛷':'sled','🥅':'goal net','⛳':'flag in hole','🎣':'fishing pole','🎽':'running shirt','🎿':'skis','🎯':'bullseye','🔮':'crystal ball','🧿':'nazar amulet','🎮':'video game','🕹️':'joystick','🎰':'slot machine','🎲':'game die','🧩':'puzzle piece','🧸':'teddy bear','🪅':'piñata','🪆':'nesting dolls','♠️':'spade suit','♥️':'heart suit','♦️':'diamond suit','♣️':'club suit','♟️':'chess pawn','🃏':'joker','🀄':'mahjong red dragon','🎴':'flower playing cards','🎭':'performing arts','🖼️':'framed picture','🎨':'artist palette',
...
'🔓':'unlocked','🔏':'locked with pen','🔐':'locked with key','🔑':'key','🗝️':'old key','🔨':'hammer','🪓':'axe','⛏️':'pick','⚒️':'hammer and pick','🛠️':'hammer and wrench','🗡️':'dagger','⚔️':'crossed swords','🔫':'water pistol','🏹':'bow and arrow','🛡️':'shield','🔧':'wrench','🔩':'nut and bolt','⚙️':'gear','🗜️':'clamp','⚖️':'balance scale','🦯':'white cane','🔗':'link','⛓️':'chains','🧰':'toolbox','🧲':'magnet','🧪':'test tube','🧫':'petri dish','🧬':'dna','🔬':'microscope','🔭':'telescope','📡':'satellite antenna',
...
// FIXED: Improved error handling and debug logging for avatar upload
function uploadMyAvatar(event) {
    ...
    const formData = new FormData();
    // Explicitly include filename for better compatibility
    ...
    fetch(apiUrl('/api/my-avatar'), {
        ...
        .then(async r => {
            console.log('Avatar upload response:', r.status, r.statusText);
            if (!r.ok) {
        ...
        .then(data => {
            ...
            // Reload so the new photo applies everywhere at once
            location.reload();
        })
        .catch(err => {
            console.error('Avatar upload error:', err);
            alert(\"Couldn't upload your avatar: \" + err.message);
        })
        .finally(() => { 
            fileInput.value = ''; 
        });
