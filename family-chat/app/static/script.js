...
function getChannelFromUrl() {
    // Set by a notification's clickAction (see send_ha_notification()
    // server-side) — best-effort, since whether this query param
    // actually survives Home Assistant's own redirect into a fresh
    // ingress session is genuinely uncertain, not something this app
    // controls either way. If it doesn't arrive intact, this simply
    // returns null and falls through to the normal stored/default
    // channel below, same as any other page load.
    try {
        const params = new URLSearchParams(window.location.search);
        const requested = params.get('channel');
        if (!requested) return null;
        // Cleaned from the address bar right away regardless of whether
        // it turned out to be a real, currently-existing channel —
        // otherwise reloading the page (or the ingress iframe simply
        // re-rendering) would keep forcing the same channel indefinitely.
        const url = new URL(window.location.href);
        url.searchParams.delete('channel');
        window.history.replaceState({}, '', url);
        if (document.querySelector(`.channel[data-channel="${CSS.escape(requested)}"]`)) {
            return requested;
        }
    } catch (e) {}
    return null;
...
function initializeChat() {
    ...
    socket.on('new_message', function(data) {
        ...
        if (wasNearBottom) {
            // scrollToBottomRobust(), not the plain scrollToBottom() —
            // this message might contain a file attachment, a GIF, or a
            // custom emoji, all of which are <img> tags that load
            // asynchronously and can grow the page after this point.
            // The plain version would set scrollTop against the height
            // as it is *right now*, landing short once that image
            // actually finishes loading in — the same bug class already
            // fixed for opening a channel, just at a different trigger.
            // Reusing the same function rather than duplicating its
            // logic a third time; its "was already at the bottom"
            // premise for the settling window holds exactly as well
            // here, since this branch itself only runs when that was
            // already true at the moment this message arrived.
    ...
    // view whether this particular client was sitting on the composer
    // view, mid-edit itself, or hadn't touched this message at all.
    socket.on('message_edited', function(data) {
        const messageDiv = document.querySelector(`.message[data-id="${data.message_id}"]`);
        if (!messageDiv) return;
        messageDiv.dataset.rawContent = data.content;

        const newTextEl = document.createElement('div');
        newTextEl.className = 'message-text';
        newTextEl.innerHTML = renderMessageText(data.content);
        const editContainer = messageDiv.querySelector('.message-edit-container');
        const existingTextEl = messageDiv.querySelector('.message-text');
        if (editContainer) {
            editContainer.replaceWith(newTextEl);
        } else if (existingTextEl) {
            existingTextEl.replaceWith(newTextEl);
        }

        // The old preview (if any) was already invalidated server-side
        // by edit_message() clearing the stored link_preview column —
        // queue_link_preview() will emit its own separate
        // link_preview_ready event afterward if the new content
        // actually has a link to preview.
        const oldPreview = messageDiv.querySelector('.link-preview-card');
        if (oldPreview) oldPreview.remove();

        // Added once, next to the timestamp — not re-added on a second
        // edit of the same message.
        const header = messageDiv.querySelector('.message-header');
        if (header && !header.querySelector('.message-edited-label')) {
    ...
    socket.on('link_preview_ready', function(data) {
        ...
        // This can arrive well after the message itself first appeared
        // — the actual preview fetch happens as a background task
        // server-side — so whether the view was sitting at the bottom
        // has to be captured fresh, right here, the same as any other
        // live content change.
        const container = document.getElementById('messagesContainer');
        const wasNearBottom = isNearBottom(container);
        ...
        // scrollToBottomRobust(), not a plain scroll — a preview card
        // can include its own thumbnail image, which loads
        // asynchronously same as any other image and can grow the page
        // further still after this point. This was previously the one
        // case with no re-scroll at all, not just one that didn't
        // account for a late-loading image within it — a card with a
        // thumbnail, title, and description showing up with nobody
        // ever scrolled down to it was the most direct version of this
        // whole bug class yet.
        if (wasNearBottom) {
            scrollToBottomRobust();
        }
    ...
    socket.on('server_error', function(data) {
        alert(data.message || 'Something went wrong. Check the app log for details.');
    });
...
function startEdit(id) {
    ...
    // Already mid-edit (a stray double-invocation) or this message has
    // no editable text element at all (shouldn't happen given the edit
    // button itself is only ever rendered for a text message, but this
    // guard costs nothing) — nothing to do either way.
    if (!textEl || msgDiv.querySelector('.message-edit-container')) return;

    const rawContent = msgDiv.dataset.rawContent || '';
    const editContainer = document.createElement('div');
    editContainer.className = 'message-edit-container';
    editContainer.innerHTML = `
        <textarea class="message-edit-textarea">${escapeHtml(rawContent)}</textarea>
        <div class="message-edit-hint">escape to <a onclick="cancelEdit(${id})">cancel</a> &middot; enter to <a onclick="saveEdit(${id})">save</a></div>
    `;
...
function cancelEdit(id) {
    const msgDiv = document.querySelector(`.message[data-id="${id}"]`);
    if (!msgDiv) return;
    const editContainer = msgDiv.querySelector('.message-edit-container');
    if (!editContainer) return;
    const textEl = document.createElement('div');
    textEl.className = 'message-text';
    textEl.innerHTML = renderMessageText(msgDiv.dataset.rawContent || '');
...
function saveEdit(id) {
    const msgDiv = document.querySelector(`.message[data-id="${id}"]`);
    if (!msgDiv) return;
    const textarea = msgDiv.querySelector('.message-edit-textarea');
    if (!textarea) return;
    const newContent = textarea.value.trim();
    // Matches the server's own rejection (see handle_edit_message) —
    // caught here too so an accidental empty save doesn't even round-
    // trip to the server before coming back as an error.
    if (!newContent) return;
    // No actual change — just close back to the display view rather
    // than sending an edit that wouldn't change anything.
    if (newContent === msgDiv.dataset.rawContent) {
        cancelEdit(id);
        return;
    }
    socket.emit('edit_message', { message_id: Number(id), content: newContent });
}
...
// content at all times rather than only when actually wanted. This
// reveals them with a tap instead — tapping a message opens its own
// action bar (closing whichever other one was open, only one shown at
// a time), and tapping anywhere else closes it again. Delegated at the
// document level for the same reason the reply-quote handler above is:
// messages come and go constantly as history loads and new ones
// arrive, so a single listener here covers all of them without needing
// to re-attach anything each time one's rendered.
//
// The .closest() checks below are what keep this from swallowing every
// other tap target a message can contain — an image, a file download
// link, a reaction pill, a reply quote, or the action buttons
// themselves all already have their own specific behavior on tap, and
// none of them should also toggle the action bar as a side effect.
// (A link inside the message text, and a link preview card, both
// already stop their own click from bubbling this far at all — see
// linkifyText() and buildLinkPreviewHtml() — so they don't need to be
// listed here too, but are included anyway as a harmless, no-cost
// safety net in case that ever changes.)
document.addEventListener('click', (e) => {
    const openMessage = document.querySelector('.message.actions-open');

    const messageEl = e.target.closest('.message');
    const hitExcludedTarget = e.target.closest(
        '.message-actions, .message-image, .file-attachment, .reaction, ' +
        '.message-reply-quote, .link-preview-card, a'
    );

    if (messageEl && !hitExcludedTarget) {
        const alreadyOpen = messageEl.classList.contains('actions-open');
        if (openMessage && openMessage !== messageEl) {
            openMessage.classList.remove('actions-open');
        }
        messageEl.classList.toggle('actions-open', !alreadyOpen);
        return;
    }

    // Landed on an excluded target within the currently-open message
    // (a reaction pill, the action buttons, etc.) — that target's own
    // handler is responsible for whatever happens next; the open bar
    // is deliberately left alone rather than closed out from under it.
    if (messageEl && messageEl === openMessage) return;

    // Anywhere else entirely — a tap on blank space, a different
    // message's excluded target, or outside the message list — closes
    // whatever was open.
    if (openMessage) openMessage.classList.remove('actions-open');
});
    ...
    fetch(apiUrl(`/api/messages?channel=${currentChannel}&before_id=${historyLoadState.oldestId}&_=${Date.now()}`))
        ...
        .then(messages => {
            ...
            if (messages.length > 0 && container) {
                ...
                // Date dividers for the whole now-taller container are
                // recomputed here, before scroll height is measured just
                // below — inserting or removing dividers changes that
                // height too, and needs to be accounted for by the same
                // scroll-position correction as the prepended messages
                // themselves, not applied separately afterward.
...
function isSameDate(timestampA, timestampB) {
    if (!timestampA || !timestampB) return false;
    const a = new Date(timestampA), b = new Date(timestampB);
...
function shouldGroupMessages(senderIdA, timestampA, senderIdB, timestampB) {
    ...
    // A same-day check on top of the time window below, not redundant
    // with it — the window alone would still group, say, an 11:58pm
    // message with a 12:02am one from the same person, which reads as
    // still "yesterday" to anyone glancing at it despite genuinely
    // being the next day.
    if (!isSameDate(timestampA, timestampB)) return false;
    const diff = Math.abs(new Date(timestampA) - new Date(timestampB));
    return diff <= GROUPING_WINDOW_MS;
}

// "Today" / "Yesterday" for the last two days, the full date otherwise
// — the same convention Discord itself uses for this.
...
function buildDateDividerHtml(timestamp) {
    return `<div class="date-divider"><span>${escapeHtml(formatDateDivider(timestamp))}</span></div>`;
}

// Used specifically after a "load older history" batch finishes (see
// loadOlderMessages()) — deliberately a full recompute across every
// currently-loaded message rather than trying to incrementally patch
// dividers message-by-message during that batch, the way grouping
// itself does. A single prepended batch can span several day
// boundaries at once, and can mean a divider that used to correctly
// sit before some message no longer belongs there at all (an even older
// message from the same day just got loaded above it) — recomputing
// from scratch sidesteps needing to reason about removing, moving, and
// adding dividers all in the same pass, at the cost of a little
// redundant work on an operation that only ever happens when someone
// deliberately scrolls up for more history, not on every message.
function refreshDateDividers(container) {
    ...
    messageEls.forEach(msgEl => {
        ...
        if (!isSameDate(lastTimestamp, ts)) {
            msgEl.insertAdjacentHTML('beforebegin', buildDateDividerHtml(ts));
        }
        lastTimestamp = ts;
    });
}
...
function addMessage(data, insertMode = 'append') {
    ...
    // The raw, unrendered text — used when this message gets edited
    // (see startEdit()), which needs the original source text back,
    // not the mentions/emoji/link-processed HTML currently on screen.
    messageDiv.dataset.rawContent = data.content || '';
    ...
    // A short month/day (no year, no weekday — that fuller form is what
    // the date divider above already shows once per day) alongside the
    // time specifically in the main header, the one shown for the
    // first message in a group. The compact, hover-only timestamp used
    // for grouped continuations (see message-timestamp-compact below)
    // deliberately keeps just the time — grouping never crosses a day
    // boundary (see shouldGroupMessages), so every grouped message
    // already shares its group's own date, and repeating it there would
    // just be clutter with no new information.
    const dateTime = new Date(data.timestamp).toLocaleDateString([], {
    ...
    // freeform text on a file, GIF, or calendar event to edit.
    const canEdit = !!data.sender_id && data.sender_id === window.AUTO_CHAT_USER_ID && msgType === 'text';
    const editBtnHtml = canEdit
        ? `<button class="action-btn" onclick="startEdit(${data.id})" title="Edit message">✏️</button>`
        : '';
    ...
    if (insertMode === 'prepend') {
        ...
        // previousSibling is reused here rather than a fresh lookup —
        // it's always either the actual previous message, or (for the
        // very first message ever rendered into this channel) the
        // welcome-message placeholder div, which has no timestamp
        // dataset at all. isSameDate()'s own null-check already treats
        // that the same as "no previous message", correctly triggering
        // a divider either way without needing to special-case it here.
        if (!isSameDate(previousSibling ? previousSibling.dataset.timestamp : null, data.timestamp)) {
            container.insertAdjacentHTML('beforeend', buildDateDividerHtml(data.timestamp));
        }
        container.appendChild(messageDiv);
    }
}
...
//
// Only scrolls again if still near the bottom at that moment, though —
// this used to force it unconditionally, which broke scrolling up
// through history entirely on a channel with older, lazy-loaded photos
// or attachments: scrolling up brings a previously off-screen, not-yet-
// loaded image into view, the browser starts loading it right then, and
// the instant it finishes, this fired and yanked the view straight back
// down — even though scrolling up was the whole point. Checking
// isNearBottom() first means a stale listener from an image nowhere
// near the bottom can never do that.
function scrollToBottomRobust() {
    scrollToBottom();
    const container = document.getElementById('messagesContainer');
    if (!container) return;
    // This only ever runs right after a fresh channel load or switch —
    // its two only call sites — so nothing here can be "the user
    // scrolled away", full stop, for a while yet: they can't have
    // deliberately moved away from content that only just appeared.
    // A generous settling window (1.5s) treats every image finishing
    // within it as still part of that initial load-in, re-scrolling
    // unconditionally — needed because a freshly-loaded, message-heavy
    // channel can easily have several avatars still loading at once,
    // and checking isNearBottom() on every single one of them
    // individually could see a gap larger than the threshold from
    // *other* images not done yet, even though the person never
    // touched the scrollbar. Past that window, this falls back to the
    // same isNearBottom() check as before — which is what actually
    // keeps a much-later image load (say, one scrolled up to and into
    // view long after this channel settled) from yanking the view
    // back down out from under someone deliberately reading history.
    const settlingUntil = Date.now() + 1500;
    const rescrollIfAppropriate = () => {
        if (Date.now() < settlingUntil || isNearBottom(container)) scrollToBottom();
    };
    container.querySelectorAll('img').forEach(img => {
        if (!img.complete) {
            img.addEventListener('load', rescrollIfAppropriate, { once: true });
            img.addEventListener('error', rescrollIfAppropriate, { once: true });
        }
    });
}
...
// Guards the same class of race switchChannel() itself is exposed to —
// see gifRequestSeq for the same pattern applied there. Clicking
// through a couple of channels quickly (an entirely normal thing to
// do) starts a fetch for each one; nothing stopped an earlier, slower
// response from resolving *after* a newer one and applying its
// messages and scroll-to-bottom on top of whatever channel is actually
// current by then.
let channelSwitchSeq = 0;

function switchChannel(slug, onLoaded) {
    const seq = ++channelSwitchSeq;
    ...
    fetch(apiUrl(`/api/messages?channel=${currentChannel}&_=${Date.now()}`))
        .then(r => r.json())
        .then(messages => {
            // A newer switchChannel() call has since started — this
            // response belongs to a channel the person already clicked
            // away from, so applying it now would splice a stale
            // channel's messages into whatever's actually open.
            if (seq !== channelSwitchSeq) return;
            messages.forEach(msg => addMessage(msg));
...
function runSearch() {
    ...
    fetch(apiUrl(`/api/search?q=${encodeURIComponent(q)}`))
        ...
        .catch(() => {
            results.innerHTML = '<p class="hint">Search failed — check the app log.</p>';
        });
}
...
function openFileBrowser() {
    ...
    fetch(apiUrl(`/api/files?channel=${currentChannel}`))
        ...
        .catch(() => {
            list.innerHTML = '<p class="hint">Failed to load files — check the app log.</p>';
        });
}
