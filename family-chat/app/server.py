...
# Home Assistant writes app options to /data/options.json regardless of
...
#
# /data survives restarts, rebuilds, and updates -- but is genuinely NOT
# guaranteed to survive an uninstall/reinstall of the app itself. This isn't
# theoretical: multiple real, documented cases exist of other apps losing
# their /data-based storage exactly this way. /addon_config (mounted here,
# inside the container, at the fixed path /config -- see config.yaml's
# addon_config map option) is Home Assistant's own officially documented
# location specifically designed to survive that, confirmed directly against
# the current developer docs rather than assumed.
#
# Falls back to /data if /config isn't actually available yet -- this is
# the normal, expected state for any install that's only been Restarted,
# not Rebuilt, since a newly added map option only takes effect on a full
# Rebuild. Continuing to work against the old location in that case (rather
# than crashing) means this update itself is safe to apply via a plain
# Restart; the migration to /config simply happens whenever the next
# Rebuild actually occurs.
_NEW_DATA_DIR = Path('/config')
_OLD_DATA_DIR = Path('/data')

def _migrate_to_addon_config():
    """Copies (never moves) existing data from /data to /config. Copying
    rather than moving means /data's original data is always still
    completely intact regardless of how this goes -- an implicit backup
    for the duration of this transition, at negligible extra disk cost for
    a chat app's typical data size. Returns True on success, False if it
    failed partway (the caller falls back to /data for this run; any
    partial copy is cleaned up here so the next start retries cleanly
    rather than mistaking a half-finished copy for a completed migration)."""
    logger = logging.getLogger(__name__)
    try:
        if (_OLD_DATA_DIR / 'chat.db').exists():
            shutil.copy2(_OLD_DATA_DIR / 'chat.db', _NEW_DATA_DIR / 'chat.db')
        for wal_suffix in ('-wal', '-shm'):
            wal_file = _OLD_DATA_DIR / f'chat.db{wal_suffix}'
            if wal_file.exists():
                shutil.copy2(wal_file, _NEW_DATA_DIR / f'chat.db{wal_suffix}')
        if (_OLD_DATA_DIR / 'uploads').exists():
            shutil.copytree(_OLD_DATA_DIR / 'uploads', _NEW_DATA_DIR / 'uploads', dirs_exist_ok=True)
        if (_OLD_DATA_DIR / 'secret_key').exists():
            shutil.copy2(_OLD_DATA_DIR / 'secret_key', _NEW_DATA_DIR / 'secret_key')
    except OSError as e:
        logger.error(
            'Migrating persistent data from /data to /config failed partway (%s) -- '
            'nothing on /data was touched, so nothing was lost. Falling back to '
            '/data for this run; migration will retry automatically next start.', e)
        shutil.rmtree(_NEW_DATA_DIR / 'uploads', ignore_errors=True)
        for fname in ('chat.db', 'chat.db-wal', 'chat.db-shm', 'secret_key'):
            (_NEW_DATA_DIR / fname).unlink(missing_ok=True)
        return False
    logger.warning('Migrated persistent data from /data to /config successfully -- this app now uses /config going forward.')
    return True
...
def _resolve_data_dir():
    ...
    try:
        ...
        logger.warning(
            '/config is not available or not writable yet -- this app needs a '
            'Rebuild (not just Restart) to pick up the new persistent-storage '
            'location. Using /data for now; nothing else changes until that '
            'Rebuild happens.')
        return _OLD_DATA_DIR

    if (_NEW_DATA_DIR / 'chat.db').exists():
        # Already migrated in an earlier run.
        return _NEW_DATA_DIR

    if (_OLD_DATA_DIR / 'chat.db').exists():
        return _NEW_DATA_DIR if _migrate_to_addon_config() else _OLD_DATA_DIR

    # A genuinely fresh install, nothing to migrate.
    return _NEW_DATA_DIR
...
# an older version of this app already created before this existed.
try:
...
# TLS upstream and proxies to this app over plain HTTP internally, so
...
# entirely — meaning the max_file_size option in the app's own
...
if not GIPHY_API_KEY:
    logger.warning('giphy_api_key is not set — the GIF picker will be disabled '
                    'until one is added in the app configuration.')

# Supervisor injects this automatically once the app has
# `homeassistant_api: true` in config.yaml, and it's what lets this app
...
# that permission hasn't been granted (or the app hasn't been rebuilt
...
if not SUPERVISOR_TOKEN:
    logger.warning('SUPERVISOR_TOKEN is not set — Home Assistant notifications will be '
                    'unavailable until this app is rebuilt with homeassistant_api access.')
...
def init_db():
    ...
    # Set the first time a message is edited (see edit_message()); NULL
    # for a message that's never been touched. Purely a "was this ever
    # edited" flag and a timestamp for the (edited) label the client
    # shows next to one — there's no history of prior versions kept
    # anywhere, so an edit is a genuine, one-way overwrite of content.
    ...
    # <family member>" without the app needing to talk to HA's auth API.
    ...
    # delivery itself is entirely Home Assistant's; this app only
...
def edit_message(message_id, requester_id, new_content):
    """Unlike delete_message(), there's no can_manage override here at
    all — only the original sender can ever edit their own message,
    regardless of admin or owner status. Silently rewriting what
    someone else appears to have said is a real trust concern in a way
    deleting a message outright isn't, so this stays strictly
    sender-only. Only a plain text message can be edited — there's no
    freeform text on a file, GIF, or calendar event to meaningfully
    change. Returns (channel, error); channel tells the caller which
    socket room to notify, same convention as delete_message()."""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT sender_id, channel, message_type FROM messages WHERE id = ?', (message_id,))
    row = c.fetchone()
    if not row:
        conn.close()
        return None, 'Message not found — it may have been deleted.'

    sender_id, channel, msg_type = row
    if sender_id != requester_id:
        conn.close()
        return None, 'You can only edit your own messages.'
    if msg_type != 'text':
        conn.close()
        return None, "That message can't be edited."

    edited_at = datetime.now().isoformat()
    # link_preview is cleared, not just left as-is — the edited content
    # may no longer contain the link it was originally built from (or
    # may now contain a different one entirely), and the caller is
    # responsible for re-queuing a fresh preview attempt against the
    # new content, the same background-task pattern used for a brand
    # new message.
    c.execute('UPDATE messages SET content = ?, edited_at = ?, link_preview = NULL WHERE id = ?',
...
def ingress_redirect(path):
    """Home Assistant ingress serves this app under a dynamic prefix
...
def set_cache_headers(response):
    # Every response from this app is either the live document itself
...
def handle_uncaught_exception(e):
    ...
    if request.path.startswith('/api/'):
        return jsonify({'error': 'Internal server error — check the app log for details.'}), 500
    return 'Internal server error — check the app log for details.', 500
...
def admin_export():
    ...
    try:
        # sqlite3's own backup API, not a plain file copy — a live
        # database can be mid-write at the exact moment this runs (this
        # app has real, concurrent traffic), and a naive copy risks
        # capturing a torn, inconsistent snapshot. backup() guarantees a
        # complete, self-consistent copy regardless of what else is
        # happening to the source at the same time.
        tmp_db_path = os.path.join(tmp_dir, 'chat.db')
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            zf.write(tmp_db_path, 'chat.db')
            if UPLOAD_FOLDER.exists():
                for root, _dirs, files in os.walk(UPLOAD_FOLDER):
                    for fname in files:
                        full_path = Path(root) / fname
                        arcname = Path('uploads') / full_path.relative_to(UPLOAD_FOLDER)
                        zf.write(full_path, str(arcname))
            zf.writestr('export_info.json', json.dumps({'exported_at': datetime.now().isoformat()}))

        timestamp = datetime.now().strftime('%Y-%m-%d_%H%M%S')
        response = send_file(zip_path, as_attachment=True,
                              download_name=f'family-chat-backup-{timestamp}.zip',
                              mimetype='application/zip')
        # send_file streams the response after this function returns, so
        # the temp directory can't be cleaned up until that's actually
        # finished — call_on_close defers it to exactly that point,
        # rather than deleting the zip out from under an in-progress
        # download or leaking it if cleanup ran eagerly instead.
...
@require_admin
def admin_import():
    """Restores a zip produced by /admin/export, entirely replacing this
    instance's current messages and uploaded files. Requires explicit
    confirmation whenever there's existing data to lose, and every
    validation step below fails closed — rejecting anything that isn't
    unambiguously a real Family Chat backup — since the whole point of
    this feature is to be trustworthy in exactly the stressful moment
    someone actually needs it."""
    # The app-wide MAX_CONTENT_LENGTH (see its own definition above) is
    # sized for a single chat attachment, governed by the max_file_size
    # option — a full backup is the *cumulative* size of the entire
    # database plus every file ever uploaded, which for any real,
    # actively-used family chat is routinely much larger than that.
    # Left unset, this route would silently inherit that same small
    # cap, meaning a real backup could fail to even upload — exactly
    # the moment this feature exists to not fail. Overridden here,
    # per-request, rather than raising the global default, since
    # nothing else needs a limit this generous.
    request.max_content_length = 2 * 1024 * 1024 * 1024  # 2GB

    uploaded = request.files.get('backup_file')
    if not uploaded or not uploaded.filename:
        return ingress_redirect(url_for('admin_panel', tab='backup', import_error='No backup file was selected.'))

    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT COUNT(*) FROM messages')
    existing_message_count = c.fetchone()[0]
    conn.close()

    if existing_message_count > 0 and request.form.get('confirm_replace') != 'on':
        return ingress_redirect(url_for('admin_panel', tab='backup',
            import_error=f'This instance already has {existing_message_count} message(s). '
                         'Check the confirmation box to replace them with the backup.'))
    ...
    try:
        ...
        if not zipfile.is_zipfile(zip_path):
            return ingress_redirect(url_for('admin_panel', tab='backup', import_error="That file isn't a valid zip."))
        ...
        imported_db_path = os.path.join(extract_dir, 'chat.db')
        if not os.path.exists(imported_db_path):
            return ingress_redirect(url_for('admin_panel', tab='backup',
                import_error="That zip doesn't contain a chat.db — not a Family Chat backup."))

        # Confirms this is genuinely a Family Chat database, not just
        # any SQLite file that happened to be named chat.db — deeper
        # schema issues (an export from a much older version, say) are
        # left to init_db()'s own existing, already-robust migration
        # logic to handle on the next start, rather than duplicated here.
        try:
            test_conn = sqlite3.connect(imported_db_path)
            test_conn.execute('SELECT COUNT(*) FROM messages')
            test_conn.close()
        except sqlite3.Error:
            return ingress_redirect(url_for('admin_panel', tab='backup',
                import_error="That chat.db doesn't look like a valid Family Chat database."))

        # Copies over the existing files rather than deleting them
        # first — if something fails partway through (disk full,
        # etc.), this instance is never left with neither the old data
        # nor a complete new copy.
        shutil.copy2(imported_db_path, DB_PATH)

        imported_uploads = os.path.join(extract_dir, 'uploads')
        if os.path.isdir(imported_uploads):
            if UPLOAD_FOLDER.exists():
                shutil.rmtree(UPLOAD_FOLDER)
            shutil.copytree(imported_uploads, UPLOAD_FOLDER)

        logger.warning('Data restored from an uploaded backup by an admin — restart the app to ensure every connection picks up the new data cleanly.')
        return ingress_redirect(url_for('admin_panel', tab='backup', import_success='1'))
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
# app just decides whether and where to call one when a message comes
...
def ha_api_request(method, path, json_body=None, timeout=5):
    ...
    if not SUPERVISOR_TOKEN:
        return None, 'This app was not built with Home Assistant API access — add "homeassistant_api: true" to its configuration and rebuild it.'
    ...
    try:
        ...
        if e.code in (401, 403):
            return None, 'Home Assistant rejected this request — the app may need to be rebuilt after enabling API access.'
...
_own_slug_cache = None

def get_own_slug():
    """This app's own installed slug (e.g. "c2f85679_family_chat" — the
    hash Supervisor derives from this repo's URL, prefixed onto the
    slug set in config.yaml), needed to build a stable link straight to
    this app for notification deep links (see notify_message_subscribers()).
    Deliberately not reusing ha_api_request() — /addons/self/info is a
    Supervisor API call, a different base URL than the Home Assistant
    Core API that function is specifically for, and — per Home
    Assistant's own developer docs — /addons/self/* is one of a handful
    of calls every app can make with just its SUPERVISOR_TOKEN,
    regardless of whether hassio_api is set. Cached after the first
    successful call, since this app's own slug can't change while it's
    running."""
    global _own_slug_cache
    if _own_slug_cache is not None:
        return _own_slug_cache
    if not SUPERVISOR_TOKEN:
        return None
    req = urllib.request.Request('http://supervisor/addons/self/info', headers={
        'Authorization': f'Bearer {SUPERVISOR_TOKEN}',
    })
    try:
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            if slug:
                _own_slug_cache = slug
            return slug
    except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, OSError, json.JSONDecodeError) as e:
        logger.warning('Could not determine this app\'s own slug (notification deep links will be skipped): %s', e)
        return None

def send_ha_notification(notify_service, title, message, channel=None):
    """Calls notify.<service> in Home Assistant. notify_service may be
    given with or without the "notify." prefix. channel, if given, is the
    chat channel this notification is about — used to build a clickAction
    so tapping the notification opens this app directly instead of
    landing on Home Assistant's main dashboard. Returns (ok, error)."""
    ...
    # Home Assistant's own API — one made with this app's Supervisor
    ...
    # of Supervisor's own request routing that this app has no
    ...
    # /hassio/ingress/<slug> is Home Assistant's own stable, session-
    # independent entry point for an ingress app — the same URL "Open
    # Web UI" itself links to — which is why it's usable here at all;
    # the actual per-session ingress proxy URL (/api/hassio_ingress/
    # <token>/...) is generated fresh by Home Assistant's frontend and
    # can't be constructed ahead of time for a notification. The
    # channel query param is attached on the chance it survives HA's
    # own redirect into that session — genuinely uncertain given how
    # that redirect is generated, so the frontend treats it as
    # optional and this degrades gracefully either way: worst case, the
    # notification still opens this app directly instead of Home
    # Assistant's main dashboard, which is the reliable part.
    own_slug = get_own_slug()
    if own_slug:
        click_action = f'/hassio/ingress/{own_slug}'
        if channel:
            click_action += f'?channel={urllib.parse.quote(channel)}'
        payload['data'] = {'clickAction': click_action}

    _, error = ha_api_request('POST', f'/services/notify/{service}', payload)
...
def notify_message_subscribers(channel, sender, sender_id, content, msg_type, file_info):
    ...
    for user_id, notify_service in subscribers.items():
        ok, error = send_ha_notification(notify_service, title, body, channel=channel)
        if ok:
    ...
    for user_id, notify_service in prefs_cache.items():
        ok, error = send_ha_notification(notify_service, mention_title, body, channel=channel)
        if ok:
...
def api_create_calendar_event():
    ...
    #
    # entity_id/calendar_name/description are included so the details
    # modal (see openCalendarEventDetails() client-side) has something
    # to show beyond the card's own title/when/location — events posted
    # before this was added just won't have these fields, which the
    # modal handles by simply omitting that row rather than erroring.
    calendar_name = next((c['name'] for c in calendars if c['entity_id'] == entity_id), '')
    event_payload = {
        'title': title, 'all_day': all_day, 'start': start, 'end': end, 'location': location,
        'entity_id': entity_id, 'calendar_name': calendar_name, 'description': description,
    }
...
def log_socket_errors(handler):
    ...
    def wrapped(*args, **kwargs):
        try:
            ...
            try:
                emit('server_error', {
                    'message': "That didn't go through — check the app log for details."
                })
            except Exception:
                pass
    return wrapped
...
def handle_edit_message(data):
    message_id = data.get('message_id')
    # Same reasoning as send_message() — identity comes from the
    # server-resolved HA session, never trusted from the client payload,
    # so nobody can edit a message by simply claiming a different
    # sender_id in the socket call.
    requester_id, requester_name = resolve_ha_identity()
    ...
    if not requester_id:
        emit('server_error', {'message': "Couldn't identify you as a Home Assistant user — try reloading the page."})
        return
    # An edit that empties a message out isn't treated as an implicit
    # delete — that's what the delete button is already for, and
    # silently repurposing an empty edit to do the same thing would be
    # surprising, undocumented behavior nobody asked for.
    if not new_content:
        emit('server_error', {'message': "A message can't be edited down to nothing — delete it instead if that's the goal."})
        return
    # Same ceiling as a brand new message (see send_message()) — kept
    # in sync with it rather than introducing a second, separate limit
    # that could quietly drift out of step with the composer's own
    # maxlength.
    if len(new_content) > 20000:
        emit('server_error', {'message': 'That message is too long (20,000 characters max).'})
        return

    channel, error = edit_message(message_id, requester_id, new_content)
    if error:
        emit('server_error', {'message': error})
        return
    ...
    # Same reasoning as send_message()'s own call — runs as a background
    # task so fetching some external site never delays the edit itself
    # taking effect, and queue_link_preview() emits its own follow-up
    # event once (if) a preview is found for whatever the message says
